    ...
    for update_item in updates.into_iter() {
        let result = match &update_item.action {
            KeylistUpdateItemAction::Add => {
                let key = if update_item.recipient_key.starts_with(DID_KEY_PREFIX) {
                    let stripped_key = update_item.recipient_key.strip_prefix(DID_KEY_PREFIX);
                    let key_result = Key::from_fingerprint(stripped_key.unwrap());
                    match key_result {
                        Ok(key) => {
